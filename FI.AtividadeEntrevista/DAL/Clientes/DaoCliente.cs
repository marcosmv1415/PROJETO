...
namespace FI.AtividadeEntrevista.DAL
{
    ...
    internal class DaoCliente : AcessoDados
    {
        ...
        internal bool VerificarDuplicidadeCpf(string CPF, long ID)
        {
            ...
            DataSet ds = base.Consultar("FI_SP_VerificaCpfPorId", parametros);
            if (ds != null && ds.Tables.Count > 0)
            {
                // Verifica se a tabela tem pelo menos uma linha
                if (ds.Tables[0].Rows.Count > 0)
                {
                    // Obtém o valor da primeira célula da primeira linha da primeira tabela
                    object value = ds.Tables[0].Rows[0][0];

                    // Converte o valor para int (se necessário)
                    int existe = Convert.ToInt32(value);

                    // Retorna true se Existe for igual a 1, indicando duplicidade
                    return existe == 1;
                }
            }
